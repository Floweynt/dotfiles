...
}:
{
  boot.initrd.postDeviceCommands = lib.mkBefore ''
    _ERASURE_MAPPER_DEVICE=/dev/mapper/${machine.luksDeviceName}
    ${builtins.readFile ./darling_erasure.sh}
  '';
  ...
  home-manager.users."${user}" = { config, ... }: {
    home.file = {
      # caches
      ".config/vesktop/sessionData/Cache/".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@caches/user/${user}/vesktop/cache/";
      ".config/vesktop/sessionData/Code Cache/".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@caches/user/${user}/vesktop/code_cache/";
      ...
      ".config/vesktop/sessionData/Local Storage/leveldb/".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@caches/user/${user}/vesktop/login_cache/";

      # config
      ".gitconfig".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@config/user/${user}/git/config";
      ".config/JetBrains/".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@config/user/${user}/jetbrains/";
      ...
      ".librewolf/c3p0ytbp.default/".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@state/user/${user}/librewolf/c3p0ytbp.default/";
      ".librewolf/Profile Groups/".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@state/user/${user}/librewolf/Profile Groups/";
      ".librewolf/profiles.ini".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@state/user/${user}/librewolf/profiles.ini";
      ".local/state/nvim/".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@state/user/${user}/nvim_state/";
      ".local/share/PrismLauncher/".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@state/user/${user}/prism_launcher/";
      ".steam".source = config.lib.file.mkOutOfStoreSymlink "/persist/@state/user/${user}/steam/root";
      ".local/share/Steam".source =
        config.lib.file.mkOutOfStoreSymlink "/persist/@state/user/${user}/steam/local_share";
