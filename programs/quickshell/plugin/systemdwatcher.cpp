...
SystemdWatcher::SystemdWatcher(QObject* parent) : QObject(parent) {
    ...
    bus.call(QDBusMessage::createMethodCall(
        SYSTEMD_SVC, MANAGER_PATH, MANAGER_IFACE, "Subscribe"));

    bus.connect(SYSTEMD_SVC, PROXY_PATH, PROPS_IFACE, "PropertiesChanged",
                this, SLOT(onPropertiesChanged(QString,QVariantMap,QStringList)));

    queryInitialState();
}

void SystemdWatcher::onPropertiesChanged(const QString& iface,
                                          const QVariantMap& changed,
                                          const QStringList& invalidated) {
    if (iface != QLatin1String(UNIT_IFACE)) return;

    if (changed.contains("ActiveState")) {
        const bool active = changed.value("ActiveState").toString() == "active";
        if (active == m_proxyActive) return;
...
void SystemdWatcher::queryInitialState() {
    QDBusMessage req = QDBusMessage::createMethodCall(
        SYSTEMD_SVC, PROXY_PATH, PROPS_IFACE, "Get");
    const QDBusMessage reply = QDBusConnection::systemBus().call(
        req << QString(UNIT_IFACE) << QString("ActiveState"));

    if (reply.type() != QDBusMessage::ReplyMessage || reply.arguments().isEmpty()) return;

    const bool active =
        reply.arguments().first().value<QDBusVariant>().variant().toString() == "active";
    if (active == m_proxyActive) return;
    m_proxyActive = active;
    emit proxyActiveChanged();
}
