...
SysmonProvider::~SysmonProvider() {
    if (m_procRingReady) ::io_uring_queue_exit(&m_procRing);
}
...
void SysmonProvider::pollCpu() {
    ...
    for (const QString& line : lines) {
        ...
        if (isAgg) {
            ...
            if (ci >= newPrevCores.size()) newPrevCores.resize(ci + 1);
            while (newCores.size() <= ci) newCores.append(0.0);
    ...
    if (anyCores) m_cpuCores = newCores;
    ...
    if (!m_cpuFreqPaths.isEmpty()) {
        qint64 sum = 0;
        for (const QString& p : m_cpuFreqPaths) sum += sysfsInt(p);
...
void SysmonProvider::pollGpus() {
    ...
    for (GpuEntry& gpu : m_gpuEntries) {
        ...
        if (gpu.active) {
            ...
            if (gpu.vramTotal == 0) gpu.vramTotal = 1;
...
void SysmonProvider::pollBatt() {
    if (m_battCapPath.isEmpty()) {
        if (m_battPct != -1.0) { m_battPct = -1; emit battUpdated(); }
        return;
    }
    bool ok;
...
bool SysmonProvider::killProcess(int pid, int sig) {
...
void SysmonProvider::pollProcs() {
    ...
    const double tickScale = (m_totalTicksSinceLastProc > 0)
        ? 100.0 * numCores / (double)m_totalTicksSinceLastProc : 0.0;
    const double memScale  = (m_memTotalKiB > 0)
        ? (double)m_pageSize / ((double)m_memTotalKiB * 1024.0) * 100.0 : 0.0;
    ...
    while ((de = ::readdir(procDir)) != nullptr) {
        ...
        for (const char* c = nm; *c; ++c) {
            if (*c < '0' || *c > '9') { pid = -1; break; }
            pid = pid * 10 + (*c - '0');
        }
        if (pid <= 0) continue;
        ProcEntry e; e.pid = pid; e.fd = -1; e.nread = 0;
        int len = std::snprintf(e.statPath, sizeof(e.statPath), "/proc/%d/stat", pid);
        if (len <= 0 || len >= (int)sizeof(e.statPath)) continue;
        entries.push_back(e);
    }
    ::closedir(procDir);
    ...
    if (m_procRingReady) {
        ...
        for (int base = 0; base < N; base += CHUNK) {
            ...
            {
                unsigned head; struct io_uring_cqe* cqe; int cnt = 0;
            ...
            for (int i = base; i < end; i++) {
                ...
                struct io_uring_sqe* sqe = ::io_uring_get_sqe(&m_procRing);
                ::io_uring_prep_read(sqe, entries[i].fd, entries[i].buf,
                                     sizeof(entries[i].buf) - 1, 0);
                ::io_uring_sqe_set_data64(sqe, (uint64_t)i);
                readCnt++;
            }
            if (readCnt > 0) {
        ...
        for (auto& e : entries) {
            ...
            ::close(fd);
            if (n > 0) { e.nread = n; e.buf[n] = '\0'; }
        }
    }

    // Phase 4: parse all buffers.
    struct ProcInfo {
        int pid; QString name; char state;
        double cpu, memPct;
    ...
    for (const auto& e : entries) {
        ...
        procs.push_back({ e.pid, name, state, cpu, memPct,
                          qint64(rss * m_pageSize / 1024),
                          qint64(vsize / 1024),
                          int(threads) });
    }
