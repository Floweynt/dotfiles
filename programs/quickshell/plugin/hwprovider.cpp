...
void HwProvider::loadSysInfo() {
    ...
    const QRegularExpression modelRe(R"(^model name\s*:\s*(.+))",
...
void HwProvider::parseIdFile(const QString& path,
...
QString HwProvider::lookupId(const QHash<QString, QString>& vendors,
    ...
    if (vn.isEmpty() && dn.isEmpty()) return vid + ":" + did;
    if (dn.isEmpty())                 return vn + " [" + did + "]";
    if (vn.isEmpty())                 return dn;
    return vn + " " + dn;
}
...
QString HwProvider::usbSpeedLabel(int mbs) {
    if (mbs >= 20000) return "USB4 Gen3×2";
    if (mbs >= 10000) return "USB 3.2 Gen2";
    if (mbs >= 5000)  return "USB 3.2 Gen1";
    if (mbs >= 480)   return "USB 2.0";
    if (mbs >= 12)    return "USB 1.1";
    if (mbs >= 1)     return "USB 1.0";
    return {};
}
...
void HwProvider::loadPci() {
    ...
    const QRegularExpression slotRe(R"(^PCI_SLOT_NAME=(.+))",
                                    QRegularExpression::MultilineOption);
    const QRegularExpression idRe(R"(^PCI_ID=([0-9a-fA-F]{4}):([0-9a-fA-F]{4}))",
                                   QRegularExpression::MultilineOption);
    const QRegularExpression drvRe(R"(^DRIVER=(.+))",
                                   QRegularExpression::MultilineOption);
    ...
    for (const QString& name : dir.entryList(QDir::Dirs | QDir::NoDotAndDotDot)) {
        ...
        list.append({
            canonical,
            slotM.hasMatch() ? slotM.captured(1).trimmed() : name,
            idM.captured(1).toLower(),
            idM.captured(2).toLower(),
            drvM.hasMatch() ? drvM.captured(1).trimmed() : QString(),
            depth
        });
    }

    std::sort(list.begin(), list.end(),
              [](const PciEntry& a, const PciEntry& b) { return a.canonical < b.canonical; });
...
void HwProvider::loadUsb() {
    ...
    for (const QString& name : dir.entryList(QDir::Dirs | QDir::NoDotAndDotDot)) {
        ...
        list.append({ hostNum, ports,
                      lookupId(m_usbVendors, m_usbIds, vid, pid),
                      usbSpeedLabel(mbs), isHub, depth });
    }

    std::sort(list.begin(), list.end(), [](const UsbEntry& a, const UsbEntry& b) {
        if (a.hostNum != b.hostNum) return a.hostNum < b.hostNum;
        const int n = qMin(a.ports.size(), b.ports.size());
        for (int i = 0; i < n; i++) {
            if (a.ports[i] != b.ports[i]) return a.ports[i] < b.ports[i];
        }
